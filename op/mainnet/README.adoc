...
+
Download a full reth snapshot (check https://datadirs.optimism.io for newer versions):
+
[source,sh]
----
wget https://datadirs.optimism.io/mainnet-reth-full-2026-02-09.tar.zst
# Much faster alternative: aria2c -x 16 -s 16 https://datadirs.optimism.io/mainnet-reth-full-2026-02-09.tar.zst
----
+
Extract snapshot directly into the volume:
+
[source,sh]
----
docker volume create op-mainnet-reth
zstd -d < mainnet-reth-full-2026-02-09.tar.zst | docker run --rm -i -v op-mainnet-reth:/data busybox tar -xvf - -C /data
----
...
# Much faster alternative: aria2c -x 16 -s 16 https://datadirs.optimism.io/op-mainnet-pre-bedrockstate.zst
